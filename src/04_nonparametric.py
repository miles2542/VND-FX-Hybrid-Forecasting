...
class ResidualHybridModel:
    ...
    def prepare_data(self, residuals_train, forecasts_val_test, lags=None):
        ...
        split_test = split_val + len(forecasts_val_test[forecasts_val_test['Set'] == 'val'])
    ...
    def train_baseline(self, res_train, pair_fcs):
        ...
        print(f"  [DEBUG] Training baseline {self.model_type.upper()} for {self.pair_name}...")
        ...
        if self.model_type == 'svr':
            ...
            self.model = MLPRegressor(hidden_layer_sizes=(100,), activation='relu', 
    ...
    def optimize(self, res_train, pair_fcs, n_trials=50, phase="global", center_params=None):
        ...
        def objective(trial):
            ...
            if phase == "global":
                trial_lags = trial.suggest_int("ml_lags", 1, self.config['data'].get('ml_lags_max', 21))
            else:
                trial_lags = center_params.get('ml_lags', self.base_lags)

            # 2. Prepare Data for this trial's lag
            X_tr_raw, y_tr_raw, _, _, _, _ = self.prepare_data(res_train, pair_fcs, trial_lags)
            ...
            if self.model_type == 'svr':
                if phase == "global":
                    ...
                    def get_range(val, factor=0.5):
                        return max(1e-5, val * (1-factor)), val * (1+factor)
                    c = trial.suggest_float("C", *get_range(center_params['C']), log=True)
                    gamma = trial.suggest_float("gamma", *get_range(center_params['gamma']), log=True)
                    epsilon = trial.suggest_float("epsilon", *get_range(center_params['epsilon']), log=True)
                ...
                if phase == "global":
                    ...
                    alpha = trial.suggest_float("alpha", center_params['alpha']*0.5, center_params['alpha']*2.0, log=True)
                    lr = trial.suggest_float("learning_rate_init", center_params['learning_rate_init']*0.5, 
                                             center_params['learning_rate_init']*2.0, log=True)
                    layers = center_params['hidden_layer_sizes']
                model = MLPRegressor(hidden_layer_sizes=tuple(layers), alpha=alpha, 
        ...
        study.optimize(objective, n_trials=n_trials, n_jobs=n_workers, callbacks=[tqdm_callback])
        pbar.close()
    ...
    def predict_and_combine(self, res_train, pair_fcs):
        ...
        X_tr, y_tr, X_val, y_val, X_te, y_te = self.prepare_data(res_train, pair_fcs, self.best_lags)
        ...
        for mode in ['arima', 'var']:
            ...
            for pair in ret_cols:
                ...
                if args.run_hpo:
                    ...
                    best_params_global = hybrid_model.optimize(res_train, pair_fcs, n_trials=30, phase="global")
                    
                    if model_type == 'mlp':
                        n_layers = best_params_global.pop('n_layers')
                        layers = [best_params_global.pop(f"n_units_l{i}") for i in range(n_layers)]
                        best_params_global['hidden_layer_sizes'] = tuple(layers)
                    
                    print(f"    Optimizing Phase 2 (Refined)... [Lags: {best_params_global.get('ml_lags', hybrid_model.base_lags)}]")
                    best_params_local = hybrid_model.optimize(res_train, pair_fcs, n_trials=20, phase="local", 
            ...
            pd.concat(all_hybrids).to_csv(os.path.join(out_dir, "forecasts.csv"), index=False)
